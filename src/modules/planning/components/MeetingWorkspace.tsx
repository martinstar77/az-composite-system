...
import { UkolPlanovani, AgendaTopic, ODDELENI_CONFIG } from "../types"
import { upsertUkol, generateMeetingOutputViaAI } from "../actions/ukoly"

interface MeetingWorkspaceProps {
  meeting: UkolPlanovani
...
export function MeetingWorkspace({ meeting, userProfiles, onSuccess, trigger }: MeetingWorkspaceProps) {
  ...
  const handleAddTopic = () => {
    ...
    const newTopic: AgendaTopic = {
      ...
      popis: "",
      prezentuje_id: meeting.vlastnik_id || null,
      doba_minut: 10,
      stav: "planned"
    }
  ...
  const handleGenerateAI = () => {
    ...
    startTransition(async () => {
      ...
      await upsertUkol({ ...meeting, agenda, zapis }, meeting.id)
        ...
        <div className="px-6 py-4 bg-zinc-900 border-b border-zinc-800 flex items-center justify-between shrink-0">
          <div className="flex flex-col gap-1 min-w-0">
            ...
            <p className="text-xs text-zinc-400">
              Datum: {meeting.datum_splatnosti ? new Date(meeting.datum_splatnosti).toLocaleDateString('cs-CZ') : "Nespecifikováno"}
              ...
              <div className="flex-1 flex overflow-hidden p-6 gap-6">
                ...
                <div className="w-[450px] flex flex-col border border-zinc-800 bg-zinc-900/20 rounded-xl p-4 overflow-hidden shrink-0">
                  ...
                  <div className="flex-1 overflow-y-auto bg-zinc-950/20 border border-zinc-850 p-3 rounded-lg">
                    {/* Render existing child tasks generated by the meeting */}
                    {/* In a real workspace, we can fetch these from client, or display a summary of created tasks */}
                    <div className="space-y-2">
                      ...
                      <div className="p-3 bg-zinc-900 border border-zinc-850 rounded-lg flex items-center justify-between">
                        <div className="space-y-0.5">
                          <p className="text-xs font-semibold text-zinc-300">Ukázka: Objednat ADR krabice</p>
                          <p className="text-[10px] text-zinc-500">Vlastník: Filip · Termín: 2. 7. 2026</p>
                        </div>
