...
import { FileText, Trash2, Download, UploadCloud, AlertCircle } from 'lucide-react'
...
export function ProductDocumentsTab({ productId, initialFiles, documentTypes }: ProductDocumentsTabProps) {
  ...
  const handleDelete = async (fileId: string, filePath: string) => {
    ...
    try {
      const { error } = await deleteProductFile(fileId, filePath, productId)
      if (error) {
      ...
      toast.success("Soubor byl úspěšně odstraněn.")
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 bg-zinc-900/40 border border-zinc-800 rounded-xl space-y-4">
        <h3 className="text-base font-semibold text-zinc-300">Nahrát nový dokument</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          ...
          <div className="space-y-2">
            ...
            <Input 
              id="customName"
              placeholder="např. Bezpečnostní list CZ 2026"
              value={customName}
        ...
        <p className="text-[10px] text-muted-foreground flex items-center gap-1">
          ...
          Podporované formáty: PDF, PNG, JPEG, WEBP. Maximální velikost 20 MB. Nahrávání probíhá do privátního úložiště.
        </p>
      </div>
      ...
      <div className="p-6 bg-zinc-900/50 border border-zinc-800 rounded-xl space-y-4 shadow-xl">
        ...
        {files.length === 0 ? (
          <div className="py-8 text-center text-zinc-500 text-sm border border-dashed border-zinc-800 rounded-lg">
            K tomuto produktu nebyly nahrány žádné dokumenty.
          </div>
        ) : (
            ...
            <table className="w-full text-sm text-left border-collapse">
              <thead>
                <tr className="border-b border-zinc-800 text-zinc-500 text-xs font-bold uppercase tracking-wider bg-zinc-900/30">
                  ...
                  <th className="p-3">Název souboru</th>
